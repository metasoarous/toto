(ns ^:no-doc ^:figwheel-always toto.app
...
(defn application [app-state]
  (if-let [spec (:view-spec @app-state)]
    ...
    [:div
      ...
      [:p "This may take a second the first time if you call a view function, unless you first call " [:code '(toto.core/start-server!)] "."]]))
...
(add-watch chsk-state ::chsk-connected?
  (fn [_ _ _ old-value new-value]
    ...
    ;; triggered in an insane loop due to existing sente reconnection attempts stacking
      ;(do
        ;(js/console.log "toto chsk closed; attempting to reestablish connection")))))
        ;(go
          ;(<! (async/timeout 2000))
