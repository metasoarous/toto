...
.PHONY: build
build:
	rm -rf resources/toto/public/js && \
	rm -rf .shadow-cljs && \
	npm install && \
	clojure -M:shadow-cljs release app lib && \
	clojure -A:pack mach.pack.alpha.skinny --no-libs --project-path target/toto.jar

.PHONY: release
release: check-clean-tree build
	# Add the js compilation output and commit
	git add resources/toto/public/js package-lock.json && \
	git commit -m "add build targets" && \
	git push origin && \
	clojure -Spom && \
	./bin/update-git-hash.clj && \
	mvn -e deploy:deploy-file -Dfile=target/toto.jar -DrepositoryId=clojars -Durl=https://clojars.org/repo -DpomFile=pom.xml
